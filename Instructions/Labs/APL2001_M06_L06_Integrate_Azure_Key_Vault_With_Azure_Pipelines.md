...
在此实验室中，你将利用在验证实验室环境时创建的服务主体，以便：
...
1. 导航到 Azure DevOps **eShopOnWeb** 项目。 转到“**管道 > 管道**”，然后选择“**创建管道**”。
...
1. 在 YAML 管道定义中，在变量部分执行以下操作：
   ...
   - 将位置变量的值设置为在本课程的上一个实验室中所使用的 Azure 区域的名称（例如 **southcentralus**）
...
1. 打开管道。 如果看到消息“此管道需要访问资源的权限，然后此运行才能继续使用 Docker Compose 部署到 WebApp”，请选择“**查看**”和“**允许**”，然后再次选择“**允许**”。 这是允许管道创建 Azure 资源所必需的。
...
1. 管道运行完成后，使用 Web 浏览器导航到 Azure 门户，打开 **rg-eshoponweb-docker** 资源组，然后选择表示管道部署的 Azure 容器注册表 (ACR) 的条目。

   > [!NOTE]
   > 若要在注册表中查看存储库，需要向用户帐户授予提供此类访问权限的角色。 你将为此使用 AcrPull 角色。
...
1. 选择“**访问密钥**”，启用“**管理员用户**”复选框，并复制将在以下任务中使用的**密码**值，就像将密码作为机密添加到 Azure 密钥保管库中一样。
...
1. 在“创建密钥保管库”边栏选项卡的“基本信息”选项卡中，指定以下设置，然后单击“下一步”：
   ...
   | 资源组 | 资源组名称 **rg-eshoponweb-docker** |
   | 密钥保管库名称 | 任何唯一的有效名称，如 **ewebshop-kv-** 后跟随机的六位数数字 |
...
1. 在“**访问策略**”部分中，选择“**+ 创建**”来设置新策略。
   ...
   - 在“**主体**”边栏选项卡上，搜索使用实验室环境 ID 或名称验证实验室环境时创建的服务主体。 再次选择**下一步**和**下一步**。
   - 在“**查看 + 创建**”边栏选项卡上，选择“**创建**”。

1. 返回到“**创建密钥保管库**”边栏选项卡，选择 **“查看 + 创建”>“创建”**

   > [!NOTE]
   > 等待预配 Azure 密钥保管库。 此过程应该会在 1 分钟内完成。
...
1. 在 YAML 管道定义中，在变量部分执行以下操作：
   ...
   - 将位置变量的值设置为之前在本实验室中使用的 Azure 区域的名称
   - 将 **YOUR-SUBSCRIPTION-ID** 替换为 Azure 订阅 ID
   - 将 **az400eshop-NAME** 替换为要部署的 Azure 容器实例的全局唯一名称，例如，字符串 **eshoponweb-lab-docker-**，后跟一个随机六位数字。 
   - 将 **YOUR-ACR** 和 **ACR-USERNAME** 替换为之前在本实验室中记录的 ACR 注册表名称。
   - 将 **AZ400-EWebShop-NAME** 替换为之前在本实验室中创建的资源组的名称 (**rg-eshoponweb-docker**)。
...
1. 在 Azure 门户中，导航到包含已部署资源的资源组 **rg-eshoponweb-docker**，然后选择“**删除资源组**”以删除在此实验室中创建的所有资源。
...
- 使用了一个 Azure 服务主体来提供对 Azure 密钥保管库机密的访问权限，以及从 Azure DevOps 访问 Azure 资源的权限。
